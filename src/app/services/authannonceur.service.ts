...
export class AuthannonceurService {
  ...
  /*getAllCampagnes() {
    return this.http.get(`${this.url}/getAllCampagnes`);
  }
  */
  getAllCampagnes() {
    console.log(this.shared.getAnnonceurToken());
  ...
  return this.http.get('http://localhost:3000/annonceur/getAllCampagnes', httpOptions);
}
  

  
  updateCampagne(nom: string, newName: string, description: string) {
    return this.http.put(`${this.url}/campagnes/${nom}`, { newName, description });
  }
  
  deleteCampagne(nom: string) {
    return this.http.delete(`${this.url}/campagnes/${nom}`);
  }
